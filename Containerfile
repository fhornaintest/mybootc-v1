...
# PREPARE PACKAGES
COPY --chmod=0644 ./system/usr__local__share__my-bootc__packages-removed /usr/local/share/my-bootc/packages-removed
COPY --chmod=0644 ./system/usr__local__share__my-bootc__packages-added /usr/local/share/my-bootc/packages-added
RUN jq -r .packages[] /usr/share/rpm-ostree/treefile.json > /usr/local/share/my-bootc/packages-fedora-bootc
...
# REMOVE PACKAGES
RUN grep -vE '^#' /usr/local/share/my-bootc/packages-removed | xargs dnf -y remove
RUN dnf -y autoremove
RUN dnf clean all

# CONFIGURATION
COPY --chmod=0755 ./system/usr__local__bin/* /usr/local/bin/
#COPY --chmod=0644 ./system/etc__skel__my-bootc /etc/skel/.bashrc.d/my-bootc
...
RUN echo "KEYMAP=fr-mac" > /etc/vconsole.conf
